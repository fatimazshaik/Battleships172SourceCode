...
//*****************************************************************************
//Defining Pin Setting Struct
typedef struct PinSetting {
    unsigned long port;
    unsigned int pin;
} PinSetting;

static const PinSetting DC = {.port = GPIOA3_BASE, .pin = 0x80}; //Pin 45
static const PinSetting OC = {.port = GPIOA0_BASE, .pin = 0x80}; //Pin 62
static const PinSetting R = {.port = GPIOA2_BASE, .pin = 0x40}; //Pin 62
...
void writeCommand(unsigned char c) {
    ...
    //DC Pin (pin 45) set to low --> transfer over to command register
    //GPIOPinWrite(GPIOA3_BASE, 0x80, 0);
    GPIOPinWrite(DC.port, DC.pin, 0);

    //Initiate MCU Communication by setting the CS pin to low
    //OLEDCS = pin number 62
    //GPIOPinWrite(GPIOA0_BASE, 0x80, 0);
    GPIOPinWrite(OC.port, OC.pin, 0);

    //Put character in SPI
    SPIDataPut(GSPI_BASE, c);

    //clean data
    SPIDataGet(GSPI_BASE,&ulDummy);

    //Stop MCU communication (CS pin to high)
    //GPIOPinWrite(GPIOA0_BASE, 0x80, 0x80);
    GPIOPinWrite(OC.port, OC.pin, OC.pin);
...
*/
    ...
    //DC Pin (pin 45) set to high --> transfer over to data register
    //GPIOPinWrite(GPIOA3_BASE, 0x80, 0x80);
    GPIOPinWrite(DC.port, DC.pin, DC.pin);
